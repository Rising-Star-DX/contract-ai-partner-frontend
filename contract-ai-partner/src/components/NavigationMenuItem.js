...
function NavigationMenuItem({
	...
	if (React.isValidElement(icon)) {
		...
		iconElement = React.cloneElement(icon, {
			style: {
				color: isActive || isActiveMain ? theme.palette.primary.main : "#ABB2B9",
			},
		});
	}
				...
				<Box sx={{pl: 12}}>
					{subMenu.map(item => (
						...
						>
							...
							<Typography
								sx={{
									color: activeKey === item.key ? theme.palette.primary.main : "#666",
									fontSize: 14,
									fontWeight: 600,
								}}
							>
	...
	return (
		<Box>
			...
			>
				...
				<Typography
					sx={{
						color: isActiveMain ? theme.palette.primary.main : "#ABB2B9",
						fontWeight: 900,
						fontSize: 16,
					}}
				>
