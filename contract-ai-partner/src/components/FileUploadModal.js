...
const FileUploadModal = ({open, onClose, onUpload}) => {
	...
	const handleRemoveFile = fileIndex => {
		setUploadingFiles(prevFiles => prevFiles.filter((_, index) => index !== fileIndex));
	};
		...
		<Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
				...
				{!loading && !error && (
					<Select
						...
						onChange={e => {
							...
							const selectedCategoryObject = categories.find(category => category.id === selectedId);
					...
					>
						...
						{categories.map(categoryObject => (
							<MenuItem key={categoryObject.id} value={categoryObject.id}>
								{categoryObject.name}
							</MenuItem>
						))}
					</Select>
					...
					<List>
						{uploadingFiles.map((item, index) => (
							...
							>
								...
								<LinearProgress variant="determinate" value={item.progress} sx={{width: "50%", ml: 2}} />
							</ListItem>
						))}
					</List>
				)}
			...
			<DialogActions>
				...
				<Button
					...
					disabled={uploadingFiles.some(file => file.progress < 100)}
				>
					닫기
				</Button>
			</DialogActions>
