...
const useDocumentList = (categoryId, fetchFunction, fetchAllFunction, mapper) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			setLoading(true);
			try {
				const data = categoryId ? await fetchFunction(categoryId) : await fetchAllFunction();
				...
				if (!data || data.length === 0) {
					console.warn("⚠️ 빈 데이터 응답! setError는 호출되지 않음.");
				}
			} catch (e) {
